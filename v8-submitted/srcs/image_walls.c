...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/27 16:00:34 by jmendes           #+#    #+#             */
/*   Updated: 2022/10/27 16:02:58 by jmendes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_img	*new_texture_image(t_window *win, char *filename)
{
	...
	if (!data)
		return (NULL);
	data->w = win;
	data->img = mlx_xpm_file_to_image(win->mlx,
			filename, &data->width, &data->height);
	if (!data->img)
		ft_exit("Error: Could not load image from xpm file\n");
	data->addr = mlx_get_data_addr(data->img,
			&data->bbp, &data->line_length, &data->endian);
	return (data);
}
